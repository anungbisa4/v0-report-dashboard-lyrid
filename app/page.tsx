...
export default function DashboardPage() {
  const allowedMemberIds = useMemo(() => {
    return membersData.map((item) => item.member.id);
  }, []);
  ...
  const analytics = useMemo(() => {
    ...
    membersData.forEach((item) => {
      memberMap[item.member.id] = item.member.first_name;
    });

    const completed = tasks.filter(
      (t) => t.state__group === "completed"
    ).length;
    ...
    tasks.forEach((t) => {
      if (t.assignee_ids) {
        t.assignee_ids.forEach((id) => {
          ...
          const stateGroup =
            t.state__group as keyof (typeof teamPerformance)[string];
          if (stateGroup in teamPerformance[id]) {
            teamPerformance[id][stateGroup]++;
          }
  ...
  const statusData = [
    ...
    {
      name: "Backlog",
      value: analytics.backlog + analytics.unstarted,
      fill: "#6b7280",
    },
  ];
  ...
  const teamPerformanceData = Object.entries(analytics.teamPerformance).map(
    ([id, stats]) => ({
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            Executive summary for stakeholders and investors
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {analytics.avgTasksPerMember} avg tasks/member
              </p>
            </CardContent>
          </Card>
                ...
                <CardContent className="pt-6">
                  <div className="flex items-start justify-between">
                    ...
                    <div
                      ...
                      style={{ backgroundColor: state.fill + "20" }}
                    >
                      <state.icon
                        className="h-6 w-6"
                        style={{ color: state.fill }}
                      />
                    </div>
                  </div>
                </CardContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="bg-card border-border">
            <CardHeader>
              ...
              <CardDescription>
                Current task allocation across states
              </CardDescription>
            </CardHeader>
        ...
        <div className="grid grid-cols-1 gap-6">
          <Card className="bg-card border-border">
            <CardHeader>
              ...
              <CardDescription>
                Task distribution by team member and state
              </CardDescription>
            </CardHeader>
            <CardContent className="h-80">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <Card className="bg-card border-border">
            <CardHeader>
              ...
              <CardDescription>
                Weekly progress over the last month
              </CardDescription>
            </CardHeader>
            <CardContent className="h-80">
              <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="space-y-2">
                ...
                <p className="text-2xl font-bold text-success">
                  {analytics.completed + analytics.started}
                </p>
              </div>
              <div className="space-y-2">
                <p className="text-sm text-muted-foreground">Backlog Items</p>
                <p className="text-2xl font-bold text-warning">
                  {analytics.backlog + analytics.unstarted}
                </p>
              </div>
              <div className="space-y-2">
                <p className="text-sm text-muted-foreground">
